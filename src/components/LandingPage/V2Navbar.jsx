...
const V2Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu - Outside nav to prevent nested fixed positioning issues */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            {/* Background Overlay */}
            ...
            <motion.div
              ...
              transition={{ duration: 0.3 }}
              className="fixed inset-0 bg-[#1a1a1a] lg:hidden z-[60]"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="absolute top-6 right-6 z-[70] text-white hover:text-[#fb8500] transition-colors duration-200"
