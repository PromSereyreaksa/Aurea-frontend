...
const PortfoliosSection = ({
  ...
  // Check if portfolio might have template/placeholder data
  const checkForTemplateData = (portfolio) => {
    ...
    const contentStr = JSON.stringify(portfolio.content || {}).toLowerCase();
    const titleStr = (portfolio.title || '').toLowerCase();
    const descStr = (portfolio.description || '').toLowerCase();

    for (const keyword of templateKeywords) {
      if (contentStr.includes(keyword.toLowerCase()) ||
          titleStr.includes(keyword.toLowerCase()) ||
          descStr.includes(keyword.toLowerCase())) {
        return true;
      }
    }

    // Also check if content is empty or minimal
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    {checkForTemplateData(portfolio) && (
                      <span
                        className="px-2 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 flex items-center gap-1"
                        title="This portfolio may contain template or placeholder text. Consider updating with real content."
                      >
                    ...
                    {openMenuId === portfolio._id && (
                      <>
                        ...
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border-2 border-neutral-200 overflow-visible z-[200]">
                          ...
                          {/* Regenerate HTML Option (only for published portfolios) */}
                          {portfolio.published && (
                            <>
                              <button
                                onClick={() => handleRegenerateHTML(portfolio)}
                                className="w-full px-4 py-3 text-left text-sm font-bold text-blue-600 hover:bg-blue-50 flex items-center gap-3 transition-all duration-300"
                              >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                </svg>
                                <span>Regenerate HTML</span>
                              </button>
                            </>
                          )}
