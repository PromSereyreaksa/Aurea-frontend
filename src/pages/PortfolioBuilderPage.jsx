...
import {
  ...
  HelpTooltip,
  AutoSaveStatus,
} from "../components/PortfolioBuilder/PortfolioBuilderUI";
...
const PortfolioBuilderPage = () => {
  ...
  const { portfolioData, setPortfolioData, isUserEditing, setIsUserEditing } =
    usePortfolioData(null);
  ...
  const { isSaving, isPublishing, save, publish } = usePortfolioSave(
    id,
    portfolioStore,
    clearDraft
  );
  ...
  useEffect(() => {
    ...
    const loadPortfolio = async () => {
      ...
      console.log(
        "🔄 loadedPortfolioIdRef.current:",
        loadedPortfolioIdRef.current
      );

      // Skip loading if we already loaded this exact portfolio ID
      // This prevents refetching after we just created the portfolio
      if (
        ...
        console.log("✅ Portfolio already loaded with this ID, skipping fetch");
        ...
        if (shouldShowSetup) {
          ...
          navigate(
            `/portfolio-builder/${id}${newSearch ? "?" + newSearch : ""}`,
            { replace: true }
          );
        }
        ...
        try {
          ...
          if (result && result.success && result.portfolio) {
            ...
            if (template) {
              ...
              if (
                !mergedContent.work?.projects ||
                mergedContent.work.projects.length === 0
              ) {
                mergedContent.work = template.defaultContent.work;
              }
              if (
                !mergedContent.gallery?.images ||
                mergedContent.gallery.images.length === 0
              ) {
    ...
    const autoTriggerTemplateSelection = async () => {
      if (
        ...
        if (templateParam && selectedTemplate.id === templateParam) {
          console.log(
            "🚀 Auto-triggering template selection for:",
  ...
  const handleTemplateSelect = async (template) => {
    ...
    if (id === "new") {
      ...
      console.log(
        "🎨 Creating new portfolio immediately with template:",
        template.id
      );
      ...
      try {
        ...
        if (result && result.success && result.portfolio?._id) {
          ...
          // Set the portfolio data immediately to prevent reload errors
          const templateData = convertToTemplateFormat(result.portfolio);
          ...
          if (template) {
            ...
            const portfolioDataWithImages = ensureValidImageUrls({
              ...templateData,
              content: mergedContent,
            });
            ...
            // Mark this portfolio as loaded to prevent refetch
            loadedPortfolioIdRef.current = newPortfolioId;
          ...
          // Use replace to avoid going back to 'new' route
          navigate(`/portfolio-builder/${newPortfolioId}?setup=true`, {
            replace: true,
          });
        ...
        toast.error(
          `Failed to create portfolio: ${error.message || "Please try again."}`
        );
      }
    } else {
      if (!portfolioData || portfolioData.templateId !== template.id) {
  ...
  const handleSetupComplete = (setupData) => {
    ...
    if (isNewFormat) {
      ...
      newDescription =
        ...
        `Portfolio created with ${selectedTemplate.name} template`;
    } else {
      ...
      contentData = {
        hero: {
          ...
          description:
            ...
            `Passionate ${personalInfo.title.toLowerCase()} with expertise in creating exceptional experiences.`,
          image: "",
        },
        about: {
          heading: "About Me",
          content:
            personalInfo.bio ||
            `I'm a ${personalInfo.title.toLowerCase()} with a passion for creating meaningful work.`,
          image: "",
          skills:
            skillsArray.length > 0
              ? skillsArray
              : ["Professional Skills", "Creative Thinking", "Problem Solving"],
        },
        ...
        contact: {
          ...
          social_links: [
            {
              platform: "Email",
              url: personalInfo.email || "your.email@example.com",
            },
            ...(personalInfo.phone
              ? [{ platform: "Phone", url: `tel:${personalInfo.phone}` }]
              : []),
            ...(personalInfo.website
              ? [{ platform: "Website", url: personalInfo.website }]
              : []),
          ].filter((link) => link.url && !link.url.includes("example.com")),
        },
      };

      newTitle = personalInfo.name
        ? `${personalInfo.name}'s Portfolio`
        : `${selectedTemplate.name} Portfolio`;
      newDescription = `Portfolio showcasing the work of ${
        personalInfo.name || "a talented professional"
      } - ${personalInfo.title || "Creative Professional"}`;
    }

    const newPortfolioData = createPortfolioFromTemplate(
      selectedTemplate.id,
      contentData
    );
